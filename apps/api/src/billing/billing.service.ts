...
export class BillingService {
  ...
  async listPurchases(tenantId: string, customerId?: string) {
    const result = await this.db.query(
      `SELECT p.id, p.customer_id, p.package_id, p.router_id, p.status, p.price, p.currency,
              p.starts_at, p.ends_at, p.created_at, c.full_name AS customer_name,
       ...
       WHERE p.tenant_id = $1 AND ($2::uuid IS NULL OR p.customer_id = $2::uuid)
  ...
  async createPurchase(tenantId: string, dto: CreateWifiPurchaseDto) {
    ...
    try {
      ...
      const pkg = await client.query(
        `SELECT id, price, currency, duration_seconds FROM packages WHERE tenant_id = $1 AND id = $2 AND is_active = TRUE`,
      ...
      const result = await client.query(
        `INSERT INTO wifi_plan_purchases (tenant_id, customer_id, package_id, router_id, price, currency, status)
  ...
  async initiatePayment(tenantId: string, dto: InitiatePaymentDto) {
    const purchase = await this.db.query(
      `SELECT id, customer_id, price, currency, status FROM wifi_plan_purchases WHERE tenant_id = $1 AND id = $2`,
    ...
    const existing = await this.db.query(
      `SELECT id, provider, provider_reference, status, amount, currency FROM payments WHERE tenant_id = $1 AND idempotency_key = $2`,
      [tenantId, idempotencyKey],
    );
    if (existing.rowCount) return existing.rows[0];

    const payment = await this.db.query(
      `INSERT INTO payments (tenant_id, customer_id, purchase_id, provider, amount, currency, status, idempotency_key)
       VALUES ($1, $2, $3, $4, $5, $6, 'PENDING', $7)
       RETURNING id, purchase_id, provider, amount, currency, status, idempotency_key, created_at`,
      [tenantId, purchase.rows[0].customer_id, dto.purchaseId, dto.provider, purchase.rows[0].price, purchase.rows[0].currency, idempotencyKey],
    );
